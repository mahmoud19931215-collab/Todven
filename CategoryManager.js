export class CategoryManager {
    ...
    selectMainCategory(cat) {
        ...
        chips.forEach(chip => { const chipCat = chip.getAttribute('data-main-cat'); if (chipCat === cat) chip.classList.add('active'); else chip.classList.remove('active'); });
        this.currentSub = null;
        this.onMainSelect(cat);
        if (cat === 'all') { this.subContainer.style.display = 'none'; this.currentSub = null; this.onSubSelect('all'); }
        else { const subs = this.subCategoriesMap.get(cat) || []; this.updateSubChips(cat, subs); }
    }
    selectSubCategory(sub) {
        if (this.currentSub === sub) return;
        this.currentSub = sub;
        const chips = this.subContainer.querySelectorAll('.chip');
        chips.forEach(chip => { const chipSub = chip.getAttribute('data-sub'); if (chipSub === sub) chip.classList.add('sub-active'); else chip.classList.remove('sub-active'); });
        this.onSubSelect(sub);
    }
    setSubCategoriesMap(map) { this.subCategoriesMap = map; }
    getCurrentMain() { return this.currentMain; }
    getCurrentSub() { return this.currentSub; }
}
