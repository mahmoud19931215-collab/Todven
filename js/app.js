...
// يجمع جميع المكونات ويدير دورة الحياة والبيانات والاتصال بالـ API
...
class App {
    ...
    async init() {
        ...
        this.categoryManager = new CategoryManager(
            ...
            (mainCat) => {
                ...
                // إذا اخترنا تصنيفاً رئيسياً غير "الكل"، نحدث التصنيفات الفرعية في المدير
                if (mainCat !== 'all') {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                ...
                if (searchStats) {
                    if (query.trim()) searchStats.innerText = `${count} نتيجة`;
                    else searchStats.innerText = '';
                }
        ...
        // إعداد مستمعي الاتصال بالإنترنت
        this.setupNetworkListeners();
        
        // إعداد مودال الإعدادات
        this.setupSettingsModal();
        
        // تحديث السلة من التخزين المحلي بعد تحميل البيانات
        setTimeout(() => {
            this.cartManager.updateFromCartItems(this.productsGrid.getAllCartItems());
        }, 500);
    }
    ...
    renderFullData(data) {
        ...
        // تحديث خريطة التصنيفات الفرعية داخل الـ CategoryManager
