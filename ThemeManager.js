...
export class ThemeManager {
    ...
    loadTheme() {
        ...
        return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : CONFIG.DEFAULT_THEME;
    }
    saveTheme(theme) { localStorage.setItem(CONFIG.STORAGE_KEYS.THEME, theme); }
    applyTheme(theme) {
        ...
        if (theme === 'dark') {
            ...
            if (this.themeIcon) { this.themeIcon.classList.remove('fa-moon'); this.themeIcon.classList.add('fa-sun'); }
        } else {
            ...
            if (this.themeIcon) { this.themeIcon.classList.remove('fa-sun'); this.themeIcon.classList.add('fa-moon'); }
        }
        this.saveTheme(theme);
    }
    toggleTheme() { this.applyTheme(this.currentTheme === 'dark' ? 'light' : 'dark'); }
    initEventListeners() { if (this.themeToggleBtn) this.themeToggleBtn.addEventListener('click', () => this.toggleTheme()); }
}
